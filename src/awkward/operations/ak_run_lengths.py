    ...
    def lengths_of(data, offsets):
        ...
        if size is not unknown_length and size == 0:
            ...
            if offsets is not None:
                ...
                diffs = backend.nplike.set_index_slice(
            ...
            full_positions = backend.nplike.set_index_slice(full_positions, 0, 0)
            full_positions = backend.nplike.set_index_slice(
                full_positions, -1, backend.nplike.shape_item_as_index(size)
            )
            full_positions = backend.nplike.set_index_slice(
                full_positions, slice(1, -1), positions + 1
            )
